...
async function cachePut(key: string, ab: ArrayBuffer): Promise<void> {
  ...
  try {
    const cache = await caches.open(TTS_CACHE_NAME);
    await cache.put(`/tts-cache/${key}`, new Response(ab, { headers: { "Content-Type": "audio/mpeg" } }));
  } catch { /* private mode */ }
}
...
async function fetchLivePost(voice: string, text: string): Promise<ArrayBuffer | null> {
  const res = await fetch("/api/tts", {
    ...
    body: JSON.stringify({ text: spokenForm(text), voice }),
  });
...
export function prefetchTTS(voice: string, text: string): void {
  ...
  for (const chunk of chunkText(text)) {
    ...
    if (bakedManifest?.has(key)) hydrateFromUrl(key, `/lesson-audio/${key}.mp3`);
    else if (chunk.length <= GET_MAX) hydrateFromUrl(key, ttsGetUrl(voice, chunk));
    else void resolveAudio(voice, chunk);
  }
}
...
function chunkText(text: string): string[] {
  ...
  for (let i = 1; i < sentences.length; i += 2) chunks.push(sentences.slice(i, i + 2).join(" "));
  return chunks;
}
...
function srcForChunk(voice: string, chunk: string): { src: string; key: string; blobUrl: boolean } {
  ...
  if (mem) return { src: URL.createObjectURL(new Blob([mem], { type: "audio/mpeg" })), key, blobUrl: true };
  if (bakedManifest?.has(key)) return { src: `/lesson-audio/${key}.mp3`, key, blobUrl: false };
  if (chunk.length <= GET_MAX) return { src: ttsGetUrl(voice, chunk), key, blobUrl: false };
...
export function useTTS(voice = "nova") {
  ...
  const prefetch = useCallback((text: string) => { prefetchTTS(voice, text); }, [voice]);
