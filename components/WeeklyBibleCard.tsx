...
export default function WeeklyBibleCard({
  ...
  // Auto-speak for xlarge (toddler) mode
  useEffect(() => {
  ...
  return (
    <div
      style={{
        background: "#fff8f2",
        borderRadius: 24,
        padding: isLarge ? "20px 16px" : "16px 14px",
        marginBottom: 20,
    ...
    >
      ...
      {/* ── For xlarge (Lois): simplified two-section view ── */}
      {isXL ? (
        <>
          {/* Big Truth (character trait toddler text) */}
          <div style={cardStyle}>
            <div style={sectionHeaderStyle}>
              <span>✨</span> This Week&apos;s Big Truth
              {speakBtn(curriculum.character.toddlerFriendly)}
            </div>
          ...
          <div style={cardStyle}>
            <div style={sectionHeaderStyle}>
              <span>📖</span> Bible Verse
              {speakBtn(curriculum.verse.text + ". " + curriculum.verse.reference)}
            </div>
            <p style={bigBodyTextStyle}>{curriculum.verse.text}</p>
            <p style={{ fontSize: 16, color: color, fontWeight: 800, marginTop: 6 }}>
        ...
        <>
          ...
          <div style={cardStyle}>
            <div style={sectionHeaderStyle}>
              <span>🙋</span> Catechism
              {speakBtn(
                `Question: ${curriculum.catechism.question} Answer: ${curriculum.catechism.answer}`
              )}
            </div>
            <p style={{ ...bodyTextStyle, fontWeight: 700, marginBottom: 4 }}>
              Q{curriculum.catechism.number}: {curriculum.catechism.question}
            </p>
            <p style={bodyTextStyle}>{curriculum.catechism.answer}</p>
            <p style={{ fontSize: 12, color: "#888", marginTop: 4 }}>
              {curriculum.catechism.reference} · {curriculum.catechism.source}
            </p>
          ...
          <div style={cardStyle}>
            <div style={sectionHeaderStyle}>
              <span>📖</span> Memory Verse
              {speakBtn(
                curriculum.verse.text + ". " + curriculum.verse.reference
              )}
            </div>
            <p style={isLarge ? bigBodyTextStyle : bodyTextStyle}>
              &ldquo;{curriculum.verse.text}&rdquo;
            </p>
            <p style={{ fontSize: 13, color: color, fontWeight: 800, marginTop: 6 }}>
             , {curriculum.verse.reference}
            </p>
            {memBtn(progress.verseMemorized, markVerse)}
          </div>
          ...
          <div style={cardStyle}>
            <div style={sectionHeaderStyle}>
              <span>🎵</span> Hymn
              {speakBtn(curriculum.hymn.verse)}
            </div>
            <p style={{ ...bodyTextStyle, fontWeight: 800, marginBottom: 4 }}>
              {curriculum.hymn.title}
            </p>
            <p style={{ ...bodyTextStyle, fontStyle: "italic", whiteSpace: "pre-line" }}>
              {curriculum.hymn.verse}
            </p>
            <p style={{ fontSize: 12, color: "#888", marginTop: 4 }}>
          ...
          <div style={cardStyle}>
            ...
            <p style={{ fontSize: 12, color: "#888", marginTop: 4, fontStyle: "italic" }}>
              &ldquo;{curriculum.character.verse}&rdquo;, {curriculum.character.reference}
            </p>
          </div>
        </>
      )}
